...
> 状态: v1.12（已完成多轮 review 与修订，达到实施标准；全部待讨论项已于 2026-08-01 确认，见 §9.10.7）
...
### 8.5 AI 审阅环节失效分析（为什么自审阅未发现 ⑪）

> 定位：§8.4 解释「自动化测试为何未发现」，§8.5 解释「人工/AI 质检环节为何也未发现」——同一批缺陷（⑪ 组合设计核心非主流宽基 / 卫星全科创 / 卫星偏少）同时逃逸了两道防线，证明缺口不止在测试断言，也在审阅心智。**与 §8.4 同源不同层**：§8.4 是自动化断言缺陷（接口形状 vs 业务语义），§8.5 是人工审阅缺陷（流程完成度 vs 业务正确性）。

#### 8.5.1 现象

初轮自审阅（§3.2）检查了：链路跑通、三套方案生成、字段完整、rationale 模板多样、层预算合规——全部 PASS。已记录 core 高度重叠、三套差异仅权重+现金等特征，但当时当作「同一预算引擎的正常产物」记录，未判定为不合格信号。用户以投资者直觉（「核心怎么全是科创小盘？」）一眼发现问题。

#### 8.5.2 根因（与 §8.4 对照）

| # | 根因 | 机制 | 与 §8.4 对应 |
|---|------|------|-------------|
| R1 | 执行者心态（完成偏差） | 审阅被当作「验证清单打勾」，目标函数是步骤完成而非缺陷发现 | G1 断言过弱：查「有」而非「合理」 |
| R2 | 确认偏差 | 对自研系统有正面预期，表面健康信号（方案生成成功 / 字段有值）抑制质疑 | G1 同源 |
| R3 | 维度错位 | 查「字段有无 / 格式合规」，不查「数值语义」：RSI 全 <3 未质疑值域、估值因子字段错位未发现、信号与因子方向不自洽未复核 | G1 值域盲区 |
| R4 | 缺领域基准 | 未先定义「合格组合」对照基准（核心含主流宽基 / 卫星行业分散 / 风格匹配市态）即下结论 | G4 无质量基线同构 |
| R5 | LLM 输出权威化 | 把报告当成品审阅，未反向验证论断与底层数据吻合（科创 50 套「压舱石」模板未抓出） | G4 LLM 非确定性 |
| R6 | 审阅不阻断 | 发现的问题记入「已知问题」继续推进，无「未达标准不得通过」闸门 | G1 断言不阻断 |

#### 8.5.3 审阅基准清单（可执行模板，实施后每次审阅照单打勾）

**A. 必含项（缺一即不合格）**

- [ ] 核心层（core）≥1 只为宽基（510300/510500/510050/588000/159915，对齐 §9.6 宽基清单）
- [ ] 卫星层行业分散：同板块 ETF ≤2 只（对齐 §9.6 步骤 C 配额）
- [ ] 卫星数量在预算区间（对齐 STRATEGY_META，6-8 只）
- [ ] 防御型方案卫星层不含 ≥25% 科创标的（C2 惩罚应生效）

**B. 反例清单（出现即不合格）**

- [ ] 核心层 3 只以上同板块 / 同风格
- [ ] 卫星层全部为科创 / 单一风格
- [ ] rationale 模板与标的属性矛盾（如科创 50 套「压舱石低波动宽基」）
- [ ] 报告论断与因子数据矛盾（如技术/估值双负仍判「偏多」）

**C. 数值语义校验**

- [ ] RSI ∈ (0, 100) 且分布合理（全 <3 或全 >97 视为数据错误）
- [ ] 估值因子与标的类型匹配（黄金 ETF 无「估值偏低」类字段错位）
- [ ] 信号方向与因子分方向自洽（技术/估值/动量同向加权，反向需注明理由）
- [ ] 三套方案差异真实（非仅权重+现金机械缩放）

#### 8.5.4 落地机制（与 T 表互补）

| 机制 | 说明 | 类型 |
|------|------|------|
| 审阅前立基准 | 先写「合格基准清单」（§8.5.3 模板）再读产出物，逐项打勾 | 流程 |
| 证伪式检查制度化 | B 类反例清单成为审阅固定环节，命中即打回 | 流程 |
| 执行者与审阅者分离 | 产出方不自审——用隔离 subagent（review / read_only_task）审报告内容而非仅代码 diff | 流程 |
| 审阅拷问 | 结论前加一问：「如果用户是专业投资者，会接受吗？」 | 流程 |
| 基准自动化 | 把 A/B/C 清单固化为可执行断言 → T14 | 测试 |
...
| T7 | factors/active **数值门限断言**（对齐 §9.5 验收）：`etf_specific no_data ≤ 2`、`sentiment no_data = 0`；`no_data` 因子 reason 必须标注缺失字段（禁止统一「尚未计算 IC」） | G1 断言过弱（②） |
| T8 | **LLM 质量金丝雀基线**（非 mock）：固定 3 条基线样本（降准/半导体停产/黄岩岛）+ 固定 prompt，断言输出①不含 system prompt 片段（含 reasoning 通道泄漏，对齐 F1-7）；②不含「未包含 XX 数据」自曝缺失模式（对齐 ⑭）；③news-impact 结论含「无直接影响」或受影响 ≤2 只（对齐 §9.9.5-2）；`test_report_quality.py` 增加 reasoning_content 泄漏用例（mock 返回 `reasoning_content` 含 prompt → 断言不进入正文） | G4 LLM 非确定性（③⑫⑬⑭） |
...
| T14 | **方案质量门禁**（§8.5.3 清单自动化）：策略检查/组合设计输出断言——①核心层 ≥1 只主流宽基（对齐 §9.6 宽基清单）；②卫星层同板块 ≤2 只；③RSI 值域校验（全 <3 或全 >97 视为数据错误）；④信号方向与因子分自洽（反向需注明理由）；⑤三套方案差异非仅权重+现金机械缩放 | G1/R1-R6（⑪ 及同类设计质量问题） |
...
第四梯队（P3-P4 — 持续）
  ...
  T1-T14 测试防护补强（§8.3/§8.4/§8.5 映射：T7/T10 随 F 修复同批，T8/T9 随 F1-7/F1-9 同批，T13 数据卫生随任意测试批次，T14 方案质量门禁随 §9.6/§9.7 修复同批）
```
...
| v1.12 | 2026-08-01 | 新增 §8.5「AI 审阅环节失效分析」：自审阅未发现 ⑪ 的 6 条根因（R1 执行者心态 / R2 确认偏差 / R3 维度错位 / R4 缺领域基准 / R5 LLM 输出权威化 / R6 审阅不阻断），与 §8.4 G1-G8 逐项对照（同源不同层）；给出可执行「审阅基准清单」模板（A 必含项 / B 反例清单 / C 数值语义校验三类，实施后每次审阅照单打勾）；§8.5.4 落地机制（审阅前立基准 / 证伪式检查 / 执行者与审阅者分离 / 审阅拷问 / 基准自动化）；T 表新增 T14「方案质量门禁」（§8.5.3 清单自动化，随 §9.6/§9.7 修复同批）；路线图 P4 同步为 T1-T14 |
