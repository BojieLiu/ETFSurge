...
1. 事实核对：所有 file:line / 数字 / commit 与代码和 git 逐一比对
   - **诊断数字方法学（DOC-1 / round56 §4.1 教训）**：行数/耗时/计数类数字必须附
     统计方法（如 `Get-Content … | Measure-Object -Line` 含空行）+ `git show <base>:<path>`
     对照值（同法复测基线 commit）；无方法标注、无基线对照的数字视为未核对——
     计数口径差异可达数百行，「继续膨胀」类趋势判断必须经同法复测确认，否则撤销加急定性。
