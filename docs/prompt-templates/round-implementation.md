...
2. **工作流**（遵循 AGENTS.md「反假完成机制」+「TDD」+「性能软门禁」）：
   ...
   - **新测试开工 10 秒预检（round53 遗留批复盘，强制——三条违规各致一轮返工）**：
     * *T4 归宿*：新用例先查并入哪个既有主题测试文件（`check_test_baseline.py`
       ...
       （round53 教训：基线门禁拦下后整体搬迁 + 处理导入冲突）；
     * *mock 规则*：用例是否触外部网络/数据源？触即 mock（AGENTS.md 硬规则）——
       真网用例在 xdist 全量下受 socket guard × 代理抖动互涉必 flaky
       （round53 教训：日历用例真网依赖 → 全量红 → 20+ 轮探因 → 第二次全量）；
     * *命名冲突*：并入宿主文件的 import 面（`from datetime import date` vs
       `import datetime` 类同名遮蔽）动手前对照，并入后统一用宿主别名。
   - **迁移/重构类任务方案先行一次成型（round53 遗留批复盘）**：先出完整迁移图
     （函数清单 + 新签名 + 全部引用点/守卫测试改法 + 导入面/re-export 清单），
     **中途不换策略**；机械改动用秒级信号验证（`ast.parse` 语法 +
     `--collect-only` + 源码守卫读取），行为测试放批尾——边迁边跑长测试 =
     反馈回路全靠分钟级兜底（round53 P2-3：守卫断言改 4 次 + 模块级/整体搬家摇摆）。
   - **flaky 探因止损（round53 遗留批复盘）**：确认失败用例属**本轮新增**且违反
     既有规则（mock/T4/timeout 纪律）→ **直接按规则重写**，不做完整根因链探查；
     根因探查留给存量测试的意外失败。
