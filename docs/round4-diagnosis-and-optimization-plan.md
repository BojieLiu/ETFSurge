...
- 实施（**定夺唯一口径**：`portfolio_service.py calculate_cumulative_pnl` R64 分支）：
  ...
  3. 019633 avg_cost=3.534 **经用户确认为真实申购成本（非录入错误）**——修复后该只按新口径正常估算（联接净值口径下盈亏率即为真实语义），无需 WARN/改动数据。
...
## 十一、实施批次与优先级（用户决策：分批实施）

```
批次 1（P0，阻断性）         ← 本批先行，verify_e2e 门禁自检（P0-3）为地基
...
每批完成标准：本批全部 P 项验收通过 + `verify_e2e.py` 全绿（P0-3 修复后）。
