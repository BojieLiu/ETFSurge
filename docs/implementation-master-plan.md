...
> ✅ **Phase 41 已完成**（2026-08-01）：实施 `docs/round2-system-diagnosis-and-optimization-plan.md` **P0 + P1 全部 12 项**（F0-3/F0-4/F0-5 + F1-1~F1-9，含 §9.6 候选池专项步骤 A-E 与 §9.7 投资合理性 R1-R5）。契约驱动 + TDD：新增 6 个测试文件 40 用例全 PASS（test_design_candidate_pool 8 / test_design_investment_quality 8 / test_llm_leak_filter 7 / test_strategy_check_timeout 5 / test_hk_realtime_fix 7 / test_llm_context_market 6 / test_f0_kline_degradation 8 / test_factor_rsi_consistency 6 / test_sector_stocks_mapping 4），全量 pytest 982 PASS（4 个失败中 3 个为基线既有、1 个分页循环 bug 已修复）；verify_e2e 全模块 **109/111**（2 个失败均为文档已记录的 P2 性能项：calculate 8.0s / HK 搜索冷启动 5.5s gate）；本地起服务实测 8 项定向验证通过。详见下方 v41.0。
...
| | | | | **T1 — nginx 拓扑层：** `verify_e2e` 新增 `nginx-proxy` 模块（`http://localhost` 的 /api 代理可达 + WS 握手 101）；本地无 nginx 时 SKIP（不 FAIL），prod（docker nginx:80）运行时做真实拓扑断言。 |
| | | | | **T2 — prod DB 完整性：** `verify_e2e` 新增 `db-integrity` 模块（sqlite 直查 `E:/ETF_Surge/data/portfolio.db`）：instruments>1000（实测 1544）、portfolio_etfs 非空（20）、watchlist 非空（2）。 |
| | | | | **T3 — 内容级断言：** ①-⑤ 中 ④搜索 SPY 首条（F3-2 轮已做）、⑤LLM 泄漏（F1-7 轮已做）已覆盖；①llm-advice 无「暂无/未知」、②llm-report HK 含恒生、③板块成分股匹配由既有测试（test_sector_stocks_mapping/test_llm_context_market）覆盖。 |
| | | | | **T4 — e2e 脚本卫生：** 过期端点清理（check_data_quality `/market/etfs`→`/market/search`、circuit-breaker `/admin/sources`→`/admin/sources/health`、5xx 列表 `/market/etfs`→`/market/sectors/heat`）；脚本头加 `sys.path.insert(0, backend)`（PYTHONPATH）；WS 缺库「跳过即 PASS」改为 **SKIP 计数**（print_summary 显示）。 |
| | | | | **T5 — factor-health 采样中位数：** live 断言改为 3 次采样取中位数（间隔 1s），修复 `ratio="23/33"` 字符串解析；IC 端点加 60s 就绪轮询（冷启动时 IC 周期未完成不误报）。 |
| | | | | **T6 — 数据源熔断演练：** 新增 `tests/test_circuit_drill.py`（3 用例）：强制 mootdx 熔断（`_cool_until` 未来）→ `registry.route` 跳过 mootdx → tencent/sina 降级输出非空；批量版同样短路；冷却期过后 mootdx 恢复优先（自愈）。 |
| | | | | **T7 — factors 数值门限：** `verify_e2e` 新增 `factor-thresholds` 模块：etf_specific no_data ≤2、sentiment no_data =0、no_data reason 禁止笼统「尚未计算 IC」（F3-4 轮已实现 reason 区分）；IC batch 未就绪（服务冷启动/数据源冷却）时 SKIP 而非误报 FAIL。 |
| | | | | **T8 — LLM 金丝雀基线：** 新增 `tests/test_llm_canary.py`（3 用例，真实 LLM + skip-if-no-key/429）：①样本输出不含 system prompt 片段（F1-7 回归）；②不含「未包含 XX 数据」自曝缺失；③news-impact 结论含「无直接影响」或受影响 ≤2 只。实测 3 用例 PASS（30s）。 |
| | | | | **T9 — 策略检查超时路径：** P1 轮已做（test_strategy_check_timeout：CancelledError 捕获/usage 留痕/fallback）。 |
| | | | | **T10 — 真实调用链分级测试：** P3 轮已做（test_news_level_classification：`_level_of` 双轨校验 + 地缘军事/停牌用例）。 |
| | | | | **T11 — 契约自动化校验：** 新增 `tests/test_contract_auto.py`（3 用例）：契约声明的每条路由必须已注册（遍历 8 个 router 模块——新版 FastAPI `_IncludedRouter` 惰性占位，`app.routes` 拿不到真实路径）；前端 `api/index.js` URL 模板指向的路由必须存在；契约文件必须含 Frontend-Backend Checklist。**校准 12 处契约-实现漂移**：circuit-breaker.md（/admin/circuit-breaker→/admin/sources/circuit-breakers，reset 删除）、ssl-connection-pool.md（/admin/sources→/sources/connection-pool）、agents.md（llm-advice 去 query、sector/symbol-analysis→stream 版、portfolio-design 删除、strategy-check→async）、llm-report.md、design.md（/design→/design-async、status 端点删除）、design-enhanced.md、strategy-check-report.md、strategy.md。 |
| | | | | **T12 — 链路两端测试：** P2/P3 轮已做（test_design_candidate_pool 候选池 fid=f6 + SectorHeatMap/NewsView specs）。 |
| | | | | **T13 — 数据卫生门禁：** `verify_e2e` 新增 `data-hygiene` 模块：watchlist 无「更新备注OK」残留（实测清理 1 条真实残留）、portfolio_designs 无 e2e 测试标记；DB 路径解析 .env 覆盖（项目根 data/portfolio.db）。 |
| | | | | **T14 — 方案质量门禁：** 新增 `app/engine/design_quality.py`（`validate_design_quality` 纯函数 5 项清单：核心层 ≥1 主流宽基 / 卫星层同板块 ≤2 / RSI 值域 / 信号自洽 / 三方案差异）+ `tests/test_design_quality_gate.py`（8 用例）+ `verify_e2e` `design-quality` 模块（最近 design 实测 5 项全过）。 |
| | | | | **验证结果：** 新增 17 用例全 PASS + T8 金丝雀 30s 真实 LLM PASS；verify_e2e 新增模块实测：db-integrity/data-hygiene/circuit-breaker 21/21 ALL PASS、factor-thresholds/design-quality PASS（IC 冷启动 SKIP 正确）；全量 pytest 待终跑确认。 |
| | | | | **改动文件：** verify_e2e.py（+5 模块）、engine/design_quality.py（新）、tests 4（新）+ contract_auto 发现、契约 11 份（T11 校准）、文档 1。 |

| | **v43.0** | 2026-08-01 | **Phase 43 — round2 方案 P3 全部 7 项实施** | 详见下方 |
| | | | | **来源：** `docs/round2-system-diagnosis-and-optimization-plan.md` 第四梯队（P3 — 持续），含 §9.5 专项（F3-4 Z04 四因子步骤 A-D）与 §9.10 专项（F3-1 新闻分级步骤 A-D）。 |
| | | | | **F3-1 — 新闻重要性分级（§9.10 步骤 A-D）：** A 词表治理——跨级去重（停牌 L5→L2、违约 L5→L3、机构名移出 L4）、L2 删高频泛词（公告/发布/数据/政策/指数/板块/业绩/财报/重组/美联储）、补地缘军事词（袭击/空袭/开战/宣战/核→L5 或 L4，冲突/军事/干预/制裁/战→L4，边境/军演/国防→L3）；B 双轨交叉校验——`_level_of` 本地分类与源 level 差值 ≥2 以本地为准 + WARNING 日志；C stars 纯语义化（`stars = level`，§9.10.7-1 已确认）；D 标题+正文前 200 字双输入（`classify_news_level(title, content)`）。 |
| | | | | **F3-2 — 搜索排序缺陷：** 跨市场合并（market=null/global）后经 `_sort_search_results` 全局排序（精确 symbol==kw 置顶，不再被 A→HK→US 段序压住）；`market=A` 个股搜索降级链改为 instruments 表 → levistock 个股（`_search_a_stocks`）→ ETF（贵州茅台可搜到）。 |
| | | | | **F3-3 — 设计现金仓位：** `STRATEGY_META.layer_budget` 三档预算和调整至 0.85（防御 50/20/15、平衡 45/30/10、进攻 40/35/10）——range_bound 市态现金由 22-32% 收紧至 **≤15%**（balanced = 15%），且保留三方案差异化（权重差 >1% 不变）。 |
| | | | | **F3-4 — Z04 etf_specific 四因子（§9.5 步骤 A-D）：** A `_fetch_market_data` IOPV 失败时天天基金日频净值降级注入 `nav`（premium_discount 复活，收盘折溢价口径）；B `_enrich_symbol_extra` 宽基 ETF（510300/510500/510050/588000/159915/510880）注入 `benchmark_close`（指数历史 close 20 日，tracking_error 复活）；C 份额数据 `fetch_etf_shares_outstanding` 注入 `shares_change_20d`（24h 缓存）+ `institutional_holdings_change` ×0.5 折扣代理（§9.10.7-5 已确认）；D `registry._data_source_gaps` 记录缺口 + `factors/active` no_data reason 区分「数据源未接入（缺字段）」vs「IC 未累积」+ `/factors/ic` 新增 `zero_ratio` 字段。 |
| | | | | **F3-5 — sentiment 3 因子：** `_fetch_market_data` 注入 `sentiment_index`/`sentiment_history`（panic_greed_diff）与 `news_items`（news_heat/news_direction）——此前只注入 refresh_pool 的 factor_scores、compute 路径拿不到 → 全 0 → IC 过滤 → 恒 no_data；注入后 compute 产出真实值进入 IC batch。 |
| | | | | **F3-6 — LLM 429 限流重试：** `LLM_MAX_RETRIES 1→2`；新增 `_rate_limit_wait`（优先尊重 Retry-After ≤30s，否则指数退避 3s·2^attempt ≤30s）；`llm_complete` 识别 429（HTTPStatusError）按退避重试；`llm_complete_stream` 外层加重试轮次循环（此前仅 provider 循环）；最终失败抛「LLM 限流，已降级」提示。 |
| | | | | **F3-7 — 自选美股名称：** `get_asset_realtime` US 分支无 name 时从静态基座 `HKUS_ETF_MAP`/`HKUS_STOCK_MAP` 补全（SPY → "SPDR S&P 500 ETF"），已有 name 不覆盖。 |
| | | | | **契约（更新 4 份）：** `factors/active-v2.md`（v3.0：no_data reason 区分 + /factors/ic zero_ratio）、`news/all.md`（v3.0：stars=level + 双输入 + 双轨校验 + 词表）、`market/hot-plates.md`（v3.0：跨市场精确匹配置顶 + market=A levistock 降级）、`portfolio/design.md`（v3.0：现金 ≤15% 预算说明）。 |
| | | | | **测试（TDD，31 新用例全 PASS）：** `tests/test_search_budget_usname.py`（7）、`test_llm_rate_limit.py`（4）、`test_factor_etf_specific.py`（7）、`test_sentiment_factors.py`（6）、`test_news_level_classification.py`（6）、`test_z29_search.py` 1 断言更新（F3-2 精确匹配置顶）；修复 mypy 2 错误（llm.py getattr 局部变量化）。 |
| | | | | **验证结果：** 全量 pytest **1028 PASS**（8 skipped）；mypy 0 错误；既有相关套件全绿（因子 117 / 新闻 46 / 搜索+设计 13）。 |
| | | | | **改动文件：** 后端 9（routers/market.py、engine/budgets.py、services/market_service.py、analysis/llm.py、factors/factor_registry.py、services/market_data_hub.py、factors/ic_tracker.py、routers/factors.py、fetchers/levistock_fetcher.py、fetchers/news_fetcher.py）、测试 5（新）+ 1（更新）、契约 4（更新）、文档 1。 |

| | **v42.0** | 2026-08-01 | **Phase 42 — round2 方案 P2 全部 9 项实施** | 详见下方 |
| | | | | **来源：** `docs/round2-system-diagnosis-and-optimization-plan.md` 第三梯队（P2 — 下迭代），含 §9.8 专项（F2-3/F2-4/F2-6/F2-7）与 §9.9 专项（F2-8/F2-9）。 |
| | | | | **F2-1 — 组合计算 8.2s→亚秒：** `portfolio_service` 新增 `_PRICE_MAP_CACHE`/`_FUNDAMENTALS_CACHE`（15s TTL，与 portfolio:realtime 一致；key=四类 symbol 列表）；`_hk_batch`/`_us_batch` 由 for 循环串行改为 `asyncio.gather` 并行；A 股 ETF 基本面（pe/pb/规模，每只 8s timeout）同样 15s 缓存——8s 主因。实测 calculate 多次采样中位数 **15ms**（验收 <2s）。 |
| | | | | **F2-2 — 首页骨架屏（CLS）：** `GlobalIndicesStrip` 新增 `loading` prop + 固定高度骨架（首屏不再渲染「暂无数据」占位后跳变）；`Dashboard` 组合 tab 内容区与 PnL 区在刷新（loading）时保持骨架（此前内容区直接消失→CLS）；路由级代码分割（router 全懒加载）/ echarts 按需注册 / manualChunks 分包 / onMounted 三 API 并行（既有，验收基础）。Lighthouse 数值（≥85/CLS<0.1）留待浏览器实测。 |
| | | | | **F2-3 — `/market/sectors/heat` 路由：** 新增 `GET /api/v1/market/sectors/heat`（`{items:[{rank,name,heat_index,rank_change,is_new,plate_code}], total}`，`plate_name→name`、`cur_heat→heat_index` 归一化）；`hub.get_sector_heat` 增加 `limit` 参数（传值时实时取数）。 |
| | | | | **F2-4 — UnifiedAnalysis SSE 接线（隐藏断裂）：** `marketApi` 补齐 `symbolAnalysis`/`sectorAnalysis`/`marketAnalysis` 方法；`UnifiedAnalysis.doAnalyze` 改用 `useLLMStream().start()`（symbol→`/symbol-analysis/stream`、sector→`/sector-analysis/stream`），**删除「✅ 查询完成」fallback 假成功分支**。 |
| | | | | **F2-5 — 预热并行：** `market_service.get_global_indices` A 股指数 / EM / HK 三段由顺序 await 改为 `asyncio.gather` 并行（函数内补 `import asyncio`，此前 NameError 被 except 吞掉）。 |
| | | | | **F2-6 — 板块字段契约修复：** 新增 `_parse_stock_list`/`_parse_concept_tags`/`_normalize_hot_plate`（`ast.literal_eval` 安全解析，绝不 eval）；`get_hot_plates` 归一化 `secu_name→name`/`up_reason→reason`/`plate_stock_up_num→stock_count`/`stock_list→lead_stocks`；`get_stock_hot_rank` enrich 补 `concept_tags` 数组；前端 stock tab 渲染 price/sector/turnover（万/亿格式化）+ 概念 chip。 |
| | | | | **F2-7 — 快速分析入口：** 新增 `TechnicalAnalysisModal.vue`（indicators+signal 弹窗，展示 RSI/MACD/KDJ/MA + 综合信号 + 转 AI 分析）；SectorHeatMap 个股行「技术分析」+「AI 分析」、板块行「AI 分析」（emit `analyze {mode,query,name}`）；`UnifiedAnalysis` 新增 `externalTrigger` prop（sector 模式外部触发）；`MarketAnalysis` 接事件后滚动到分析区并触发；`analysis.py` 新增 `_normalize_sector_code`（名称优先 → cls 数字段匹配 BK），映射失败 404 结构化错误。 |
| | | | | **F2-8 — 资讯 AI 分析行内展开：** `NewsView` 删除页面底部面板，改为卡片内行内展开（`impactTarget` + loading + 结果 + 失败行内重试 + 再次点击 toggle；「仅有结果时 toggle」避免失败态误收起）；按钮禁用防竞态。 |
| | | | | **F2-9 — 新闻分析质量约束：** `analyze_news_impact` prompt 追加硬约束「若新闻与组合内标的无直接关联，须明确回答『无直接影响』，禁止强行关联；只列出实际受影响的标的，宁缺毋滥」（持仓与空组合两个分支均覆盖）。 |
| | | | | **契约（更新 4 份）：** `market/hot-plates.md`（v2.0：sectors/heat `{items,total}` + 归一化字段 + concept_tags）、`market/market-context.md`（sector-analysis cls 归一化 + 404 结构化错误）、`portfolio/calculate.md`（F2-1 15s 缓存说明）、`analysis/agents.md`（news-impact「无直接影响」约束）。 |
| | | | | **测试（TDD，24 新用例全 PASS）：** 后端 `tests/test_sector_heat.py`（6）、`test_portfolio_calc_parallel.py`（3）、`test_warmup_parallel.py`（1）、`test_news_impact_quality.py`（3）；前端 `src/test/SectorHeatMap.spec.js`（6）、`src/test/NewsView.spec.js`（5）；既有回归套件全绿（相关后端套件 56 PASS / 全前端 284 PASS）。 |
| | | | | **验证结果：** 全量 pytest **998 PASS**（8 skipped，3 deselected）；verify_e2e portfolio 模块 **52/54**（仅剩 design-async/strategy-check 的 LLM 429 限流 Z07 已知项，P3 F3-6 范围）；前端 `npm run build` 通过；本地起服务实测：calculate 中位数 15ms、sectors/heat 200（name/heat_index/rank_change）、hot-plates 11 行（name/reason/stock_count/lead_stocks[29]）。 |
| | | | | **改动文件：** 后端 7（routers/market.py、services/market_data_hub.py、routers/analysis.py、services/portfolio_service.py、services/market_service.py、analysis/llm.py、scripts/verify_e2e.py）、前端 8（api/index.js、components/market/UnifiedAnalysis.vue、components/market/SectorHeatMap.vue、components/market/TechnicalAnalysisModal.vue（新）、views/MarketAnalysis.vue、components/NewsView.vue、views/Dashboard.vue、components/GlobalIndicesStrip.vue）、测试 6（新）、契约 4（更新）、文档 2。 |

| | **v41.0** | 2026-08-01 | **Phase 41 — round2 方案 P0+P1（12 项）实施** | 详见下方 |
| | | | | **来源：** `docs/round2-system-diagnosis-and-optimization-plan.md`（P0 阻断 + P1 数据质量，含 §9.6 候选池专项步骤 A-E / §9.7 投资合理性 R1-R5）。 |
| | | | | **F0-3 — nginx WS 代理修复：** `frontend/nginx.conf` 新增 `location /api/v1/ws`（置于 `/api` 之前，upgrade/Connection 头 + 3600s read timeout），生产模式 WS 不再被 HTTP 代理吞掉（前后端代码零改动）。 |
| | | | | **F0-4 — A 股 K 线降级链增强：** `fetch_history` A 股日线补**网易财经兜底**（mootdx→sina→netease）；`market_service.get_history` 全源失败时从 hub **过期 K 线缓存兜底**（`get_kline_rows_any`/`get_kline_age_seconds`/`mark_kline_stale`）+ WARNING 日志；`/indicators`、`/signal` 响应附加 `_stale` 标记（数据新鲜度可观测）。 |
| | | | | **F0-5 — 候选池修复（§9.6 步骤 A-E）：** A `_fetch_em_etf_list` `fid=f3→f6`（成交额排序）+ 分页失败重试 1 次不静默丢页；B `WIDE_BASIS_STATIC` 主流宽基静态兜底注入 `full_pipeline`（510300/510500/510050/588000/159915/518880/511090），使 `CORE_REQUIRED/DEFENSE_REQUIRED` 真正生效；C 卫星层科创系配额（名称含 科创/半导体/芯片/AI 合计 ≤ 预算 50%，防御型 40% 收紧至验收线 10%）；D 卫星数量下限 ≥4（概念组去重/配额裁剪后实际入选 <4 时从 core 未入选按 composite 补足，绕过 segment 去重、用剩余预算、防御型排除科创）；E C2 惩罚触发条件修正（`_valuation_is_meaningful`：黄金/债券等无估值概念资产的错位估值分不再视为有效 → 防御型科创惩罚 -1.5 正常触发）。 |
| | | | | **F1-1/F1-2 — 港股/单只 A 股 realtime 降级链：** 新增 `_normalize_hk_symbol`；`_sina_realtime` HK 用 `rt_hk` 前缀（此前 `sz00700` 恒空）、`_tencent_realtime` HK 用 `hk` 前缀 + 返回符号与请求一致（含 .HK）、`_em_hk_realtime` 归一化匹配；`fetch_a_stock_realtime` 补 tencent 分支（mootdx→tencent→sina，与批量版对齐）。 |
| | | | | **F1-3/F1-4 — LLM 上下文与 market 参数：** `build_full_context` 按 market 分支取指数（A 本地缓存 / HK→全球指数「港股」区域 / US→「美股」区域），非 A 市场跳过板块采集；`strategy_design._build_market_context` 指数缓存为空时全球指数兜底 + `benchmark_stocks` 龙头股填充（此前恒空）。 |
| | | | | **F1-5 — 设计因子 RSI 口径一致：** `factor_definitions.yaml` rsi_14/rsi_24 `standardization: zscore→raw`（保留真实 0-100，此前 z-score×5 使方案内 RSI 全部 <3 与 /indicators 不一致）；`aggregate_factor_scores` valuation 聚合排除纯价格键 `etf.price`（保留 `etf.price.*` 子键如股息率），消除黄金类假估值分。 |
| | | | | **F1-6 — 板块成分股映射：** `_ak_sector_stocks` 先建行业/概念 代码→名称 映射（`stock_board_industry/concept_name_em`），用**名称**查询成分股（akshare 接口接收名称非 BK 代码，此前传 BK0447 返回错位数据），industry 失败降级 concept。 |
| | | | | **F1-7 — LLM 输出泄漏过滤：** 新增 `strip_internal_leak` 纯函数（已知系统提示词泄漏模式整行/行内剔除）；`llm_complete_stream` 只取 content 通道（**丢弃 reasoning_content**——推理模型复述 system prompt 的泄漏源）；`llm_complete`/`llm_complete_with_system` 返回值统一过滤。 |
| | | | | **F1-8 — 投资合理性（§9.7）：** R3 `rationale.py` 新增 `_style_probe` 四类短语池（低波宽基/高波成长/主题卫星/防御资产），科创指数强制归高波成长池（绝不「压舱石/低波动」），短语完整句式一次生成（修「在方案中在方案中」重复与截断）；R5 `signal.py` 新增 `composite_signal`（0.4技术+0.4估值+0.2动量加权 + 单因子 |score|≤1.0 封顶 + 「技术<0 且 估值<0 → 至多 hold」双弱不判多），rationale 综合信号改用它。 |
| | | | | **F1-9 — 策略检查超时假象：** `generate_strategy_check_report` 改捕获 `BaseException`（含 CancelledError——wait_for 超时取消内部任务时它不被 `except Exception` 捕获 → 失败记录写不进 usage、fallback 从未轮到）；`portfolio_service` wait_for 捕获 `(TimeoutError, CancelledError)` + **usage 失败记录** + 超时文案带耗时（「LLM 分析超时（Xs 未返回，已用规则引擎兜底）」区分真超时与规则兜底）。 |
| | | | | **契约：** 更新 3 份 — `market/all.md`（§10 变更记录：HK 符号归一化降级链 / indicators+signal `_stale` 标记 / history stale 缓存兜底）、`analysis/llm-report.md`（§5.1 market 参数分支表 + LLM 输出过滤）、`portfolio/design-enhanced.md`（F1-3 index 兜底 + `benchmark_stocks` 契约）。 |
| | | | | **测试（TDD，新增 9 文件 58 用例全 PASS）：** `tests/test_design_candidate_pool.py`（8）、`test_design_investment_quality.py`（8）、`test_llm_leak_filter.py`（7）、`test_strategy_check_timeout.py`（5）、`test_hk_realtime_fix.py`（7）、`test_llm_context_market.py`（6）、`test_f0_kline_degradation.py`（8）、`test_factor_rsi_consistency.py`（6）、`test_sector_stocks_mapping.py`（4）；另修 `verify_e2e.py` 2 处脚本断言与契约不一致（daily-pnl 补 `total_capital`、risk_warnings 类型集合补 `general`）。 |
| | | | | **验证结果：** 全量 pytest `-m "not slow"` **982 PASS**（4 failed 中 3 个为基线既有已 `git stash` 验证、1 个分页循环 bug 已修复）；verify_e2e 全模块 **109/111**（2 个失败均为文档已记录的 P2 性能项：`/portfolio/calculate` 8.0s gate、`/market/search?market=HK` 冷启动 5.5s gate）；本地起服务定向实测 8 项通过：港股 00700.HK realtime 200 有价、A 股 510300 realtime ×10 无 null、indicators/510300 RSI=43.4（真实 0-100）、history/510300 240 行、BK0447 成分股 146 只、llm-report market=HK 报告含恒生指数、design 三套方案 core 含主流宽基 + 卫星=4 + 防御型科创权重=10%（验收线内）+ rationale 无模板缺陷 + market_context 非空。 |
| | | | | **已知遗留（本次范围外，文档已记录）：** `/factors/active` summary 统计口径（3 个 static 因子致 `valid+warn+no_data != total`，基线失败）；LLM 间歇 429 限流（Z07，P3 F3-6）；`/portfolio/calculate` 8s 并行化（P2 F2-1）；HK 搜索冷启动（P2）。 |
| | | | | **改动文件：** 后端 13（routers/market.py、routers/analysis.py、services/market_service.py、services/market_data_hub.py、services/llm_context.py、services/strategy_design.py、services/portfolio_service.py、fetchers/china_market.py、fetchers/etf_scanner.py、fetchers/sector_fetcher.py、engine/allocation_engine.py、engine/rationale.py、analysis/signal.py、analysis/llm.py、factors/factor_registry.py、factors/factor_definitions.yaml、scripts/verify_e2e.py）、前端 1（nginx.conf）、契约 3（更新）、测试 9（新）、文档 1（implementation-master-plan.md）。 |
