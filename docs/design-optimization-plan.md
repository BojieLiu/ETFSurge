...
以下为当前 session 发现的其余问题，尚未实施修复。

### 8.1 P4 — PoolManager 数据管道断裂（1549→0）

| 环节 | 状态 | 说明 |
|:----|:----|:-----|
| **P4-a**: `filter_etfs` 列名匹配 | ❌ 未修复 | `_decode_df` 后列名仍为乱码（latin1），`_get_col` 查找的 `"成交额"` 与实际乱码列名不匹配，导致 `amount` 返回 0，全部 ETF 因 `amount < MIN_AVG_AMOUNT(1000万)` 被过滤 |
| **P4-b**: 禁止静默降级 | ❌ 未修复 | `filter_etfs: 1549 → 0` 只写了 `logger.info`，无 WARNING，开发者和用户都不知道候选池异常 |
...
| 优先级 | 问题 | 预估改动 |
|:-----:|:----|:--------|
| **P4-a** | filter_etfs 列名匹配 | 1 文件 ~15 行 |
| **P4-b** | 静默降级 → ERROR 日志 | 跨 3 文件 ~30 行 |
| **P4-c** | `_warnings` 后端字段 | 1 文件 ~5 行 |
| **P4-d** | `_warnings` 前端展示 | 1 文件 ~20 行 |
| **P5-a** | Prompt 规则1 强化 | 1 文件 ~15 行 |
| **P5-b** | 一致性校验 ERROR 日志 | 1 文件 ~3 行 |

**合计**：6 个问题，8 个文件改动，~90 行净改动。
