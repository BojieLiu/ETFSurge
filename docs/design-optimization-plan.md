...
> 版本: v2.0 | 日期: 2026-07-18 | 覆盖全链路问题，含已修复 + 待修复
...
> 以下问题清单首次编写于 v1.0。后续 Phase 0.5/0.7/0.8/0.9/1.0 已修复部分问题。保留列表以供追溯。

### 8.1 P4 — PoolManager 数据管道断裂（1549→0）

| 环节 | 状态 | 说明 |
|:----|:----|:-----|
| **P4-a**: `filter_etfs` 列名匹配 | ✅ **已修复** | `_normalize_columns()` + `_get_col()` 多别名匹配已在 Phase 0.5 落地，`amount=0` 时自动跳过金额过滤 |
| **P4-b**: 禁止静默降级 | ✅ **已修复** | `filter_etfs` 增加了 `amount > 0` 守卫，避免 amount=0 导致全量过滤（Sina 源无此字段时不会误杀） |

**影响**：全市场 1549 只 ETF 被过滤至 0，PoolManager → FactorRegistry → ETFClassifier 全链路失效，系统静默降级到 `CANDIDATE_POOL` 的 18 只硬编码 ETF + 7 只硬编码卫星 ETF。

**诊断证据**：
```
raw_etfs count: 1549            ← akshare 正常返回
filtered count: 0               ← 全部被过滤
first row keys (garbled): ['����', '����', '���¼�', ...]
```

**文件**：`backend/app/fetchers/etf_scanner.py` — `fetch_all_etfs_base`, `filter_etfs`, `_get_col`

**修复方案**：
1. `fetch_all_etfs_base` 返回后加列名校验：若关键列名（如 `"成交额"`）不存在，输出 ERROR 日志 + 原始列名列表
2. `filter_etfs` 入口检测：若 `amount` 全部为 0 → ERROR 日志 + 拒绝降级（或至少 WARNING + 明确标识）
3. `_get_col` 增加更宽容的匹配（如模糊匹配、latin1 编码匹配）

### 8.2 P4-c/d — 候选池异常的前端感知

| 环节 | 状态 | 说明 |
|:----|:----|:-----|
| **P4-c**: 后端传递 `_warnings` | ❌ 未修复 | `market_context` 中增加 `_warnings` 字段，候选池为空时设置 `_warnings: ["候选池数据异常，卫星标的可能不完整"]` |
| **P4-d**: 前端展示 banner | ❌ 未修复 | 方案详情页顶部展示黄色 warning banner |

### 8.3 P5 — LLM 报告质量增强

| 环节 | 状态 | 说明 |
|:----|:----|:-----|
| **P5-a**: Prompt 规则1 强化 | ❌ 未修复 | 当前规则1「不修改方案数据」不够强硬，LLM 仍认为自己是策略师。需改为「严格复制方案数据，不得以任何理由重新设计，不得新增/删除/替换 ETF 代码」 |
| **P5-b**: 一致性校验 ERROR 日志 | ❌ 未修复 | `_validate_report_consistency` 发现 LLM 引入候选池外 ETF 时，应输出 ERROR 日志（而非仅追加脚注），便于排查 |

### 8.4 执行优先级（v4.0 更新）

| 优先级 | 问题 | 状态 | 预估改动 |
|:-----:|:----|:----:|:--------|
| **P4-a** | filter_etfs 列名匹配 | ✅ **已修复** | ~15 行（已落地） |
| **P4-b** | 静默降级 → ERROR 日志 | ✅ **已修复** | ~30 行（已落地） |
| **P4-c** | `_warnings` 后端字段 | ❌ 未实施 | 1 文件 ~5 行 |
| **P4-d** | `_warnings` 前端展示 | ❌ 未实施 | 1 文件 ~20 行 |
| **P5-a** | Prompt 规则1 强化 | ❌ 未实施 | 1 文件 ~15 行 |
| **P5-b** | 一致性校验 ERROR 日志 | ❌ 未实施 | 1 文件 ~3 行 |

**合计**：6 个问题中 2 个已修复，4 个未实施，~90 行净改动。
