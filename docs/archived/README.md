...
## 归档原则
- **已实施完成的计划**：如 `round2`-`round6` 各轮诊断与优化计划（round6 的 F/R 项全部落地，commit `0c78db8`）——活跃计划见 `docs/round7-rediagnosis.md`
...
- **不归档**：`api-contracts/`（活跃契约）、`backend/app/analysis/prompts/`（运行时）、`.sisyphus/`（工具私有状态）、README/AGENTS（项目说明）、`docs/round7-rediagnosis.md`（待实施计划）
