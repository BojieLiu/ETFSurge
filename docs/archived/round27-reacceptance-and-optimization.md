...
| R46 | P1 | R40 首启空窗：live 源失败 → 永无快照可读 | **根因更正（用户追问驱动）**：live 源失败的**真因是动量计算用错数据源**——`_compute_industry_momentum`（`market_trends.py:79`）/`_compute_concept_momentum`（`:117`）用 `ak.stock_board_industry_name_em`（akshare **硬编码 `17.push2.eastmoney.com`，实测 ProxyError 阻断**），而项目自有 `fetch_em_industry_sectors`（`sector_fetcher.py:426`，走 `EM_PUSH_HOST`=push2delay，**实测 200 OK 返回 496 行** BK1201 电子+1.78%）。修复：①**主修复**——`_compute_industry_momentum`/`_compute_concept_momentum` 改调项目自有的 push2delay fetcher（`fetch_em_industry_sectors`），live 源恢复正常 → 快照正常写入 → 首启空窗从根上消失；②快照 as_of 非今日时，设计链路/报告显式标注（诚实呈现快照时效）；③写侧已放宽「非空即写」（R40-b），保留读取兜底（R40-a 已做） | ①单测（抓假负向）：mock akshare 阻断 + 注入 push2delay 496 行 → `compute_sector_momentum` 非 []（禁再「akshare 阻断即返空」）；②盘后首启 `verify_e2e` 设计链路 `sector_momentum` 非 []；③快照 as_of 非今日时前端/报告显式标注 | `market_trends.py:79/:117`、`sector_fetcher.py:426`（fetch_em_industry_sectors）、`market_data_hub.py:1658/:1363` |
...
| R46 | 探针已通过（push2delay 496 行，push2 阻断） | 1658/1363、market_trends:79/117、sector_fetcher:426 | 是（盘后） | 是（push2delay 真源） | 设计强板块注入 | as_of 标注 | 复用既有 fetcher | 用错数据源（akshare push2 阻断） |
...
- **Round 9（R46 根因更正，用户追问驱动，本次完成）**：用户追问「板块快照兜底是 push2 和 push2delay 两个域名都不可用吗？」触发。实测：`push2.eastmoney.com` 与 `17.push2.eastmoney.com`（akshare 硬编码）均 **ProxyError 阻断**，但 **`push2delay.eastmoney.com` 200 OK**。**纠正 Round 8 的 R46 探针误判**——之前探的是 `ak.stock_board_industry_name_em`（akshare 硬编码 17.push2，阻断），误判「东财板块整体阻断、改新浪行业 spot」；实为**项目自有 `fetch_em_industry_sectors`（`sector_fetcher.py:426`，走 push2delay）实测返回 496 行**（BK1201 电子+1.78%）。更深根因：`_compute_industry_momentum`（`market_trends.py:79`）/`_compute_concept_momentum`（`:117`）**用 akshare 硬编码 push2（阻断）而非项目自有 push2delay fetcher**——这正是 warmup `_compute_industry_momentum failed: Connection aborted` 的真凶。R46 修复更正为「动量计算改用项目自有 push2delay fetcher」，live 源恢复正常、快照正常写入、首启空窗从根上消失。已更新 §15.1 R46 根因 + 修复 + D1-D8 表。

> **当前状态（Round 1-9 完成）**：R42-R55 均达实施标准（精确 file:line + 修复片段 + 验收 + 测试断言）；R42 由「多选项」升级为「方案(a) 决定」；R43/R48 file:line 经核查修正；R46/R53/R55 三个 D1 探针**已执行并有明确结论**（R46=push2delay 496 行可用、push2 阻断；R53=新浪 .INX 5693 行；R55=K线 240 天）；R48 采纳用户决策「三型均合并留一」；R52/R53/R54/R55 为用户 review 驱动新增。本文档**不写任何修复代码**，等待「开始实施」指令。
