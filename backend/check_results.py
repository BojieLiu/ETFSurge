...
with open('E:\\ETF_Surge\\backend\\prompt_results_all.json', encoding='utf-8') as f:
    d = json.load(f)
c = [r for r in d if r.get('analysis',{}).get('total_etf_count',0) > 0]
print(f'Total: {len(c)} completed out of total')
if d:
    best = max([r for r in d if r.get('analysis',{}).get('total_etf_count',0) > 0], 
