...
cur.execute('SELECT name FROM sqlite_master WHERE type="table" AND name="portfolio_etfs"')
if cur.fetchone():
...
print("\nInserting 红利低波ETF holdings...")

# Holdings list
holdings = [
    # 512890 is the NEW 红利低波ETF (华泰柏瑞中证红利低波动ETF)
    # This is an on-exchange fund that tracks 中证红利低波动指数
    (512890, '华泰柏瑞中证红利低波动ETF', 'A', 0.065, 'on_exchange', '红利低波ETF', 1),
    
    # Other domestic ETF holdings (preserved)
    ...
    # 007467 is the NEW 红利低波ETF联接C (华泰柏瑞中证红利低波动ETF联接C)
    # This is an off-exchange fund that tracks 000300 (沪深300)
...
# Note: 512890 (华泰柏瑞中证红利低波动ETF) is a domestic ETF
# It doesn't have a tracked_index field as it's tracked by its own index
...
for row in cur.fetchall():
    print(f"  ID: {row['id']:2d} | Symbol: {row['symbol']:6s} | {row['name'][:15]:15s} | Type: {row['portfolio_type']:12s} | Tracked: {row['tracked_index']}")

# Summary
print("\n=== Summary ===")
on_exchange_count = len([h for h in holdings if h[4] == 'on_exchange'])
off_exchange_count = len([h for h in holdings if h[4] == 'off_exchange'])
print(f"Total holdings: {len(holdings)}")
print(f"On exchange: {on_exchange_count}")
print(f"Off exchange: {off_exchange_count}")
print(f"Off exchange with tracked_index: {len([h for h in holdings if h[4] == 'off_exchange' and str(h[0]) in ('7467', '007467')])}")
...
print("=" * 60)
print("\nKey changes:")
print("  1. ✓ Added tracked_index column to portfolio_etfs table")
print("  2. ✓ Replaced 红利ETF with 红利低波ETF (512890 on_exchange)")
print("  3. ✓ Replaced 红利ETF联接C (012762) with 红利低波ETF联接C (7467 off_exchange)")
print("  4. ✓ 007467 tracks 000300 (沪深300) for预估收益")
print("  5. ✓ All existing holdings preserved")
print("\nNow the dashboard should display:")
print("  • 红利低波ETF (512890) and 红利低波ETF联接C (007467)")
print("  • 红利低波ETF联接C will use 000300 (沪深300) for estimates")
print("  • Cash holdings will be calculated correctly")
print("=" * 60)