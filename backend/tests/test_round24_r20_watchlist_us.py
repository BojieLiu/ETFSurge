...
class TestWatchlistUsRealtimeUnavailable:
    """R20: US/HK 自选实时不可用 → 显式降级标识 + T-1 收盘兜底。"""

    @pytest.mark.asyncio
    async def test_us_null_realtime_flagged_unavailable(self, monkeypatch):
        """US 标的实时 → None → realtime_unavailable=true + realtime_note（负向：静默
        null 无标注 → FAIL）。"""
        ...
        async def _fake_get_asset_realtime(symbol, asset_type):
            return None

        async def _fake_last_close(symbol, asset_type):
            calls["last_close"] += 1
            return {"price": 500.0, "is_estimated": True, "estimate_source": "last_close"}

        monkeypatch.setattr(ms, "get_asset_realtime", _fake_get_asset_realtime)
        monkeypatch.setattr(ms, "_last_close_fallback", _fake_last_close)

        out = await ms._enrich_watchlist_item(_Item())
        assert out["realtime_unavailable"] is True
        assert "realtime_note" in out
        assert calls["last_close"] == 1, "US 实时不可用应尝试 T-1 收盘兜底"
        assert out["realtime"]["price"] == 500.0
        assert out["realtime"]["is_estimated"] is True

    @pytest.mark.asyncio
    async def test_us_realtime_price_none_also_flagged(self, monkeypatch):
        """realtime 对象存在但 price=None（数据源返回空壳）→ 同样标不可用。"""
        from app.services import market_service as ms
        ...
        async def _fake_get_asset_realtime(symbol, asset_type):
            return {"symbol": "AAPL", "price": None, "change_pct": None}
        ...
        out = await ms._enrich_watchlist_item(_Item())
        assert out["realtime_unavailable"] is True
        # realtime 为空壳（price=None）时保留原对象、不重复拉收盘——前端据
        # realtime_unavailable 显示「暂无实时」，不误当实时价
        assert out["realtime"]["price"] is None

    @pytest.mark.asyncio
    async def test_a_share_null_not_flagged(self, monkeypatch):
        """A 股实时不可用保持原行为（无 realtime_unavailable）——负向：A 股误报 → FAIL。"""
        from app.services import market_service as ms
        ...
        monkeypatch.setattr(ms, "get_asset_realtime", _fake_get_asset_realtime)
        # 不 mock _last_close_fallback —— A 股路径不应调用它

        out = await ms._enrich_watchlist_item(_Item())
        assert "realtime_unavailable" not in out
        assert out["realtime"] is None
    ...
    async def test_hk_null_flagged_and_fallback(self, monkeypatch):
        ...
        out = await ms._enrich_watchlist_item(_Item())
        assert out["realtime_unavailable"] is True
        assert out["realtime"]["price"] == 461.6
        assert out["realtime"]["as_of"] == "2026-08-14"
