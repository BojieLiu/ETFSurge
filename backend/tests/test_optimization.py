...
def test_get_realtime_batch_cache_hit():
    async def _run():
        await memory_cache.clear()
        with patch("app.fetchers.china_market.fetch_a_stock_batch") as mock_batch:
...
class _FakeETF:
    def __init__(self, symbol, asset_type="A", name=None, portfolio_type="on_exchange", tracked_index=None):
        self.symbol = symbol
...
def test_get_portfolio_realtime():
    async def _run():
        ...
        with patch("app.services.portfolio_service.list_etfs") as mock_list, \
             patch("app.fetchers.china_market.fetch_a_stock_batch") as mock_batch, \
             patch("app.fetchers.china_market.fetch_index_realtime") as mock_idx:
...
def test_resample_4h():
    ...
    out = china_market._resample_4h(rows)
    assert len(out) == 2
    assert out[0]["最高"] == 3
    assert out[0]["最低"] == 0
    assert out[0]["收盘"] == 4
