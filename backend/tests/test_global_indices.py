...
# ── Fix 1: S&P 500 Sina code（合并自 test_global_indices_fix.py）─────────
...
# ── Fix 2: Sina page scrap for European indices（合并自 test_global_indices_fix.py）─
...
# ── P0-20 (round16 3.21, 自 test_p020_indices_meta.py 并入): HK 指数补全 ─────────
# 验收: ① collect_all 含「恒生港股通」系列 ≥3（负向：0 命中 → FAIL）；
#       ② HK 指数 ≥60 条（新浪 38 + 静态段）；③ 美股主流指数（SPX/道琼斯/纳斯达克）入表；
...
class TestP020StaticIndexBackfill:
    ...
    def test_static_extra_has_hs_connect_series(self):
        """静态兜底段含「恒生港股通」系列 ≥3。"""
        static, _ = self._collect_all_static()
