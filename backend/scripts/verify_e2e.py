...
def section_factors(host, port):
    ...
    # 它们被 P1-C 正确标 warn 警示（建议降权/淘汰）是预期行为；门禁目标是防
    # 「负 IC 标 valid + 无警示」的静默——即负 IC 若仍标 valid（无 reason）才 FAIL。
    try:
        ...
        if _ra2.status_code == 200:
            ...
            for _cat in _data2.get("categories", []):
                for _f in _cat.get("factors", []):
                    if (_f.get("ic_value") is not None
                            and _f.get("ic_value", 0) < 0
                            and abs(_f.get("ic_value", 0)) >= 0.05):
                        if _f.get("status") == "valid":
                            _neg_valid.append(f"{_f.get('code')}={_f.get('ic_value')}")
                        elif _f.get("status") == "warn":
                            _neg_warn += 1
            check("P3-E 无静默负 IC 活跃项（负 IC 标 valid 且无警示）",
                  not _neg_valid,
                  f"负 IC 仍标 valid（静默）: {_neg_valid[:4]}" if _neg_valid else f"负 IC 均已标 warn 警示（{_neg_warn} 项）")
        else:
