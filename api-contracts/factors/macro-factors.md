...
> **用途**: 4 个 MARKET_LEVEL 类宏观因子（月频 3 + 季频 1），注册进因子模型，
...
**功能描述 / Description**: 注册 4 个宏观环境因子 + 扩展 LLM 上下文宏观段。
...
1. `factor_registry.py` `_BUILTIN_COMPUTERS`：4 个 compute 函数注册（code → fn）
2. `routers/factors.py` `MARKET_LEVEL_FACTOR_CODES`：4 个 code 加入（否则 `/factors/active` 不会以 static 标注）
