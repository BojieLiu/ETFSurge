...
> **容器全链路诊断（触发词）**：用户说 `容器全链路诊断`（可带 round 文档路径，如
> `容器全链路诊断，round 文档 = docs/round31-xxx.md`）时，读取
> `docs/prompt-templates/container-fullchain-diagnosis.md` 并按模板执行
...
## 分析结果质量审查（四问法，诊断/验收必做）

**适用**：组合设计、策略检查、各分析报告（LLM 或规则生成）的每一条结论——**逐句审查**，不做整体笼统评估。

- **四问逐条过**：
  1. **事实 or 推断？**——事实可查证（数据中能直接找到），推断需支撑；
  2. **推断的支撑在哪？**——对应到具体因子值/权重/实测相关性/行情数据（`file:line` + 数值），无支撑 = 臆断（例：round30 港股「相关性较高」无实测支撑，kline_cache 实测 513120×512890 仅 +0.120）；
  3. **有无反例或内部矛盾？**——自相矛盾 = 不可信（例：策略检查「13/13 无兜底」vs「33.3% 覆盖」）；
  4. **与当下行情一致吗？**——跨数据源核对；陈旧/占位数据下的判断 = 失效（例：红利/恒科偏弱仅有技术面信号、无因子支撑，且与 KDJ J 90.11 vs 6.90 冲突）。
- **结论分级**：合理 / 部分合理（缺支撑）/ 臆断（无支撑）/ 失效（陈旧或占位数据）。
- **数据准确性抽查**：权重之和 = 1−现金；报告数值 vs DB 源数据；占位值检测（RSI 50.0 / 动量 +0.300 / ln_mcap 0.0）；`as_of`/`data_source`/`estimate_source` 新鲜度。
- **输出格式**：判断质量矩阵（判断原文 | 事实/推断 | 支撑 `file:line`+数值 | 与当下行情一致? | 结论分级 | 修复建议）+ 汇总（可采信 N / 需修正 M / 臆断 K）。
...
- **必做（软）**：
  1. **关键路径耗时自测**：改动涉及到 watchlist / search / design / factor-health / symbol-analysis 等热点路径时，交付前量一次耗时（`time curl` 或 verify_e2e 已有 timer / `scripts/verify_perf.py` 软门禁），对照基线记录到提交说明/issue；
  2. **复杂度审计**：新增网络/DB/文件调用必须`asyncio.wait_for`/超时参数或批量/缓存——**无超时的外部调用是"空转源"**（mootdx 8s 空转教训）；循环内 IO 需批量化或缓存；
  3. **设计评审复杂度审计**（round15 process-review P5）：**设计阶段**即检查——新增网络/DB 调用是否有超时/批量/缓存；循环内是否 IO；不满足的设计不进实施清单（非新增门禁段，保持软门禁性质）；
  4. **不静默降级**：慢路径**不得用假数据/半真值兜底冒充正常**（属上节反假完成）；性能不足时**诚实地标注**（如「数据源慢，已降级」而非伪造值）。
...
## 设计流程（round15 process-review 落地，设计评审必查）

**每份方案文档/设计评审对照 `docs/design-checklist.md` 8 项清单**（可行性探针 / 证据链 / 验证窗口 / 非兜底数据 / 真实调用点 / 四态 UI / 复杂度审计 / 已知问题模式）。三条约定的落地形态：

- **可行性探针前置**（D1）：方案写进文档/计划前，先用 10 行级脚本或单接口探测验证核心假设（数据源可达、协议匹配、字段存在）；**探测克制**（单次探测 + 失败后 ≥60s 间隔，防触发限流）。探针失败 → 方案不进实施清单。
