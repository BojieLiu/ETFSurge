...
多资产实时行情分析与 ETF 组合管理系统，覆盖 **A 股、港股、美股、黄金、原油、白银** 六类资产，内置 AI 组合设计、实时因子模型与 LLM 投资分析，行情数据层全部基于免费数据源。
...
免费行情数据天生不可靠。akshare、新浪、腾讯、东财、财联社、mootdx——每个源大部分时间可用，又各自以不同方式失灵：限流、冷却窗口、连接被断、个别标的查不到。ETF Surge 把「免费数据不可靠」当作要解决的核心问题，而不是一句免责声明。

因此，**容灾是整个工程的第一优先**：

- 每类资产都有独立的**降级链**，统一经过熔断路由（指数退避，冷却 60s → 600s 封顶）。
- 处于冷却期的源直接跳过，第一个返回有效数据的源胜出。
- **空结果记为「未命中」而非「失败」**——健康的数据源不会被「查无此标的」污染熔断状态。
- 所有数据源都不可用时，接口**明确降级**——降低精度、返回净值估算值、或标注 `数据源不可用`，绝不返回过期或伪造数据。

数据层之上，是一套与数据层截然相反的策略引擎：**纯函数、零 I/O、完全确定性**。生产环境跑的引擎和单测里跑的，是同一份代码。
