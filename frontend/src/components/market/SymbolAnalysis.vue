<template>
  <AppCard variant="outlined" title="个股/ETF 分析" description="技术图表、指标叠加与 AI 标的研报" icon="📈" class="symbol-analysis">
    <template #default>
      <!-- Search & Controls -->
      <Section :padded="false" :divided="true" title="标的搜索">
      ...
      <!-- No Selection State -->
      <Section v-if="!selectedSearchItem && !searchQuery" :padded="false" class="empty-prompt">
        <span class="prompt-icon" aria-hidden="true">💡</span>
        <p>搜索并选择一个标的开始分析</p>
      </Section>

      <!-- Chart Section -->
      <Section v-if="selectedSearchItem" :padded="false" :divided="true" title="{{ selectedSearchItem.name }} ({{ selectedSearchItem.symbol }})">
        ...
        <AppSkeleton v-if="loading" type="chart" height="480" />
        <div v-else-if="!chartData" class="empty-prompt"><p>暂无数据</p></div>
        <div v-else :style="{ height: faChartHeight + 'px' }" ref="chartContainerRef" class="chart-area">图表区域 (ECharts)</div>
      </Section>

      <!-- Indicators -->
      <Section v-if="indicatorData" :padded="false" :divided="true" title="技术指标">
      ...
      <!-- Signal -->
      <Section v-if="signal" :padded="false" :divided="true" title="综合买卖信号">
        <div class="signal-content">
          <AppBadge :variant="signal?.signal === 'buy' ? 'success' : signal?.signal === 'sell' ? 'danger' : 'warning'" class="signal-badge">
      ...
      <!-- AI Report -->
      <Section v-if="symbolReport" :padded="false" title="🤖 AI 分析">
      ...
      <!-- AI Report Loading/Error -->
      <Section v-if="symbolLoading" :padded="false" title="🤖 AI 分析">
        <AppSkeleton type="text" :rows="8" />
      </Section>

      <Section v-if="symbolError && !symbolLoading" :padded="false" title="🤖 AI 分析">
        <AppAlert variant="danger" :closable="false">
...
import { AppCard, AppButton, AppInput, AppSelect, AppSelect, AppSkeleton, AppAlert, AppBadge, Section } from '@/components'
